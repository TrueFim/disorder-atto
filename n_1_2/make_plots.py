...
if __name__ == "__main__":
    ...
    sorted_folders = sorted(folders, key=lambda s: (extract_c_num(s), extract_s_num(s), extract_k_num(s)))
    ...
    ims_current = sorted(ims_current, key=lambda s: (extract_c_num(s), extract_s_num(s), extract_k_num(s)))
    
    ims_all = [name for name in os.listdir('ims/all') if name.lower().endswith('.png') and os.path.isfile(os.path.join('ims/all', name))]
    ims_all = sorted(ims_all, key=lambda s: (extract_c_num(s), extract_s_num(s), extract_k_num(s)))
