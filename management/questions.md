...
- Does the coherence time $\tau_\mathrm{coh}$ (Mandel's definition) correlate with the momentum relaxation rate $\gamma$? (Both arise from scattering off the disordered potential, but probe different physics: interband coherence vs. intraband transport.) *Preliminary result (2026-02-18): No correlation with Mandel's metric. Requires further investigation with alternative coherence time definitions.*
- Is there probe-induced dephasing? Does the probe pulse accelerate decay of $J_z(t)$ beyond what is observed in pump-only simulations? Is this effect present in perfect crystals as well, or is it disorder-specific? *Preliminary result (2026-02-18): Probe-induced acceleration of $J_z$ dephasing observed for at least one disordered case; crystal comparison pending.*
...
- After the pulse, FID in our TDDFT simulations decays via inhomogeneous broadening (unitary evolution in a static potential). But does the spectral width that governs this decay reflect the spatial delocalization of the e-h pair in the disordered potential — i.e., are Purschke et al.'s spatial decoherence length $\Delta_0 \approx 6a_0$ (arXiv:2511.14678) and our temporal $\tau_\mathrm{coh}$ two descriptions of the same physics? Furthermore, does the spatial decoherence mechanism play an additional role *during* the light-matter interaction, where the probe actively drives e-h separation? The pump-only vs. pump-probe comparison of $J_z(t)$ can test the latter.
- Is an exponential fit to the local maxima of the autocorrelation of $J_z(t)$ (pump-only) a more physically informative coherence time metric than Mandel's $\tau_\mathrm{coh}$ computed from $J_x(t)$ (pump-probe)? Trofim's preliminary result suggests this alternative metric correlates with $\gamma$, but the method requires validation (elimination of manual outlier exclusion, applicability to the perfect crystal case).
