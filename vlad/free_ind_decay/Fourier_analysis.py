...
def _(all_data, np, pump_pulse_t0, scipy, subtract_non_FID_signal):
    ...
    def select_relevant_data_and_fix_time_axis(
        all_data, t0, subtract_non_FID_signal
    ):
        all_relevant_data = {}
        for dir1 in all_data:
            ...
            for dir2 in all_data[dir1]["data"]:
                ...
                if subtract_non_FID_signal:
                    ...
                    t1 = Drude_t[i1]
                    y1 = Drude_fit[i1]
                    dydt1 = (Drude_fit[i1 + 1] - Drude_fit[i1]) / (
                        Drude_t[i1 + 1] - Drude_t[i1]
                    )
                    ...
                    interpolator = scipy.interpolate.CubicSpline(
                        Drude_t, Drude_fit, extrapolate=True
                    )
                    s = t_grid >= t1
    ...
    FID_data_time_domain = select_relevant_data_and_fix_time_axis(
        all_data, pump_pulse_t0, subtract_non_FID_signal
    )
...
def _(FID_data_time_domain, Fourier_transform, au, np, soft_window):
    ...
    def Fourier_transform_relevant_data(
        all_relevant_data, omega_array, softening_time
    ):
        ...
        for dir1 in all_relevant_data:
            ...
            for dir2 in all_relevant_data[dir1]:
                ...
                # plt.show()
                # # TEST END
                transformed_data = Fourier_transform(
                    t,
                    data[:, 1] * window,
                    omega_array,
                    is_periodic=False,
                    pulse_center_times=0.0,
                )
                Fourier_transformed_data[dir1][dir2] = transformed_data
        return Fourier_transformed_data


    omega_array = np.linspace(1.0, 8.0, 500) / au.eV / au.fs
    FID_data_frequency_domain = Fourier_transform_relevant_data(
        FID_data_time_domain, omega_array, softening_time=10.0
    )
...
def _(
    ...
    def perform_visualization1(
        time_domain_data, omega_array, frequency_domain_data
    ):
        with PdfPages("cryst_vs_displ.pdf") as pdf:
            for dir2 in sorted(time_domain_data["cryst"]):
                ...
                axs[0].set_title(
                    "Electric current after subtracting wave packet motion"
                )
                ...
                for n, dir1 in enumerate(("cryst", "displ")):
                    ...
                    ax.plot(
                        ...
                        Y,
                        color=colors[n],
                        label=labels[n] + r"; $T_2 \geq {:.1f}$ fs".format(T2),
                    )
    ...
    perform_visualization1(
        FID_data_time_domain, omega_array, FID_data_frequency_domain
    )
    return
    ...
    def perform_visualization2(
        ...
        for n, dir in enumerate(sorted(time_domain_data["amorph"])):
            ...
            ax.plot(
                ...
                Y,
                color=colors[n],
                label=dir + r"; $T_2 \geq {:.1f}$ fs".format(T2),
            )
            ax.set_xlabel("time (fs)")
        # axs[2].set_ylim(max(-20, Y_min), min(20, Y_max))
        axs[2].set_xlim(0, 15)
    ...
    def perform_time_frequency_analysis(time_domain_data):
        for dir1 in time_domain_data:
            for dir2 in time_domain_data[dir1]:
                X = 1e-15 * np.ascontiguousarray(
                    time_domain_data[dir1][dir2][:, 0]
                )
