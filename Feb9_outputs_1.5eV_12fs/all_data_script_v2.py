...
if len(sorted_folders)<12:
    ...
    for (a0, b0, c0) in arr_abc:
        ...
        if num_folder!=0:
            ...
            [1/effecive_mass_model(a, a0, b0, c0) for a in np.linspace(alphas_start[num_folder-1], 
...
for (a0, b0, c0) in arr_abc:
    ...
    effective_mass_vals = [1/effecive_mass_model(a, a0, b0, c0) for a in np.linspace(alphas_start[num_folder-1], 
...
for folder in sorted_folders:
    ...
    plt.plot(t[m0_cut_4:], (Jm_pump_z[m0_cut_4:]-j_fit_full)/(y_fit+max(fid_peak_vals[-4:])-C**2), color='m')
    plt.grid()
    plt.xlabel('Time [fs]', fontsize=xlabel_fontsize)
    ...
    normalized_fid_signal=(Jm_pump_z[m0_cut_4:]-j_fit_full)/(y_fit+max(fid_peak_vals[-4:])-C**2)
...
# columns = ["energies_eV",  "F_fid", "F_fid_normalized"]
# df = pd.DataFrame(spectral_intensity_data.T, columns=columns)

# df.to_csv("all_data/fid/z/data/" + str(num_folder)+ "_spectral_intensity_" + sorted_folders[num_folder-1] + ".csv", index=False)

# with open("all_data/fid/z/data/" + str(num_folder)+ "_spectral_intensity_" + sorted_folders[num_folder-1] + ".csv", "w") as f:
#     f.write(r"# Units: Energy [eV], Spectral intensity [normalized] " + "\n")
#     df.to_csv(f, index=False)
