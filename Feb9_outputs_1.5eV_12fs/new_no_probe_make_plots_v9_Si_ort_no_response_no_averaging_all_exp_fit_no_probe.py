...
def plot_nonlinear_response_v4(folder, ax, j):
    ...
    product_delta_x = El_f_probe_x * (Jm_both_x - Jm_pump_x) * volume
    # Разность по времени
    dt = np.diff(t, prepend=t[0])  # prepend чтобы сохранить размер
    ...
    delta_Jm_x = Jm_both_x - Jm_pump_x
    ...
    # Разностный ток по x
    plt.subplot(5, 2, 8)
    plt.plot(t, (Jm_both_x - Jm_pump_x), color='#00FF7F')
    ...
    # Разностный ток по y
    plt.subplot(5, 2, 10)
    plt.plot(t, (Jm_both_y - Jm_pump_y), color='#FF007F')
    plt.xlabel('Time [fs]')
    plt.ylabel('J [$fs^{-1} \\cdot Å^{-2}$]')
...
folders = [f for f in folders 
           if f.startswith('c') 
           and os.path.isfile(os.path.join(f, 'pump_pulse_rt.data'))]
